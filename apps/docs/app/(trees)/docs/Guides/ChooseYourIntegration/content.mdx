`@baguette-studios/trees` exposes one path-first model and two primary runtime
entries: a thin React layer in
[`@baguette-studios/trees/react`](#get-started-with-react) and the vanilla class
in [`@baguette-studios/trees`](#get-started-with-vanilla).
